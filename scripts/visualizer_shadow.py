...
def construct_meshes_per_object(obj_key, raw_data, set_color, obj_name, ambient_object, use_texture=False):
    ...
    robot = URDF.load("data/body_models/shadow/shadowhand.urdf")

    joint_list = ["x_joint", "y_joint", "z_joint", "x_rotation_joint", "y_rotation_joint", "z_rotation_joint", "FFJ4", "FFJ3", "FFJ2", "FFJ1", "MFJ4", "MFJ3", "MFJ2", "MFJ1", "RFJ4", "RFJ3", "RFJ2", "RFJ1", "LFJ5","LFJ4", "LFJ3", "LFJ2", "LFJ1", "THJ5", "THJ4", "THJ3", "THJ2", "THJ1"]

    v3d_r1 = np.zeros((num_frames, 3477, 3))
    v3d_r2 = np.zeros((num_frames, 8, 3))
    v3d_r3 = np.zeros((num_frames, 746, 3))
    v3d_r4 = np.zeros((num_frames, 207, 3))
    v3d_r5 = np.zeros((num_frames, 297, 3))
    v3d_r6 = np.zeros((num_frames, 297, 3))
    v3d_r7 = np.zeros((num_frames, 297, 3))
    v3d_r8 = np.zeros((num_frames, 297, 3))
    v3d_r9 = np.zeros((num_frames, 8, 3))
    v3d_r10 = np.zeros((num_frames, 549, 3))
    v3d_r11 = np.zeros((num_frames, 549, 3))
    v3d_r12 = np.zeros((num_frames, 549, 3))
    v3d_r13 = np.zeros((num_frames, 549, 3))
    v3d_r14 = np.zeros((num_frames, 633, 3))
    v3d_r15 = np.zeros((num_frames, 224, 3))
    v3d_r16 = np.zeros((num_frames, 224, 3))
    v3d_r17 = np.zeros((num_frames, 224, 3))
    v3d_r18 = np.zeros((num_frames, 224, 3))
    v3d_r19 = np.zeros((num_frames, 486, 3))
    v3d_r20 = np.zeros((num_frames, 166, 3))
    v3d_r21 = np.zeros((num_frames, 486, 3))
    v3d_r22 = np.zeros((num_frames, 166, 3))
    v3d_r23 = np.zeros((num_frames, 486, 3))
    ...
    for i in range(num_frames):
        ...
        f3d_r22 = link_list[21].faces
        temp_verts = np.ones((link_list[21].vertices.shape[0], 4))
        temp_verts[:, :3] = link_list[21].vertices
        temp_verts = np.matmul(temp_verts, pose_list[21].T)[:, :3]
        v3d_r22[i, :, :] = np.matmul(temp_verts, rot_mat[i]) + np.tile(trans_r.numpy()[i, :].reshape(1, 3), (temp_verts.shape[0], 1))

        f3d_r23 = link_list[22].faces
        temp_verts = np.ones((link_list[22].vertices.shape[0], 4))
        temp_verts[:, :3] = link_list[22].vertices
        temp_verts = np.matmul(temp_verts, pose_list[22].T)[:, :3]
        v3d_r23[i, :, :] = np.matmul(temp_verts, rot_mat[i]) + np.tile(trans_r.numpy()[i, :].reshape(1, 3), (temp_verts.shape[0], 1))
    ...
    v3d_r22 -= centers_o.numpy()
    v3d_r23 -= centers_o.numpy()
    ...
    meshes['right22'].rotation = [[1, 0, 0], [0, 0, 1], [0, -1, 0]]
    meshes['right23'].rotation = [[1, 0, 0], [0, 0, 1], [0, -1, 0]]
...
def parse_args():
    ...
    parser.add_argument("--seq_name", type=str, default="shadow_WineGlass")
