...
class ObjectTensors(nn.Module):
    def __init__(self, use_texture=False):
        super(ObjectTensors, self).__init__()
        self.obj_tensors = thing.thing2dev(construct_obj_tensors(OBJECTS, use_texture), "cpu")
        self.dev = None
...
def construct_obj(object_model_p, use_texture):
    # load vtemplate
    mesh_p = op.join(object_model_p, "mesh.obj")
    parts_p = op.join(object_model_p, f"parts.json")
    if use_texture:
        mesh_p = mesh_p.replace(".obj", "_tex.obj")
        parts_p = parts_p.replace(".json", "_texture.json")
...
def construct_obj_tensors(object_names, use_texture):
    obj_list = []
    for k in object_names:
        object_model_p = f"./data/arctic_data/data/meta/object_vtemplates/%s" % (k)
        obj = construct_obj(object_model_p, use_texture)
        obj_list.append(obj)
